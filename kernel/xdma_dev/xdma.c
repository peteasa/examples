...
{
	printk(KERN_DEBUG "<%s> xdma_filter: chan_id %d\n", MODULE_NAME, chan->chan_id);
	printk(KERN_DEBUG "<%s> xdma_filter: chan->private 0x%x\n", MODULE_NAME, chan->private);
	printk(KERN_DEBUG "<%s> xdma_filter: param 0x%x\n", MODULE_NAME, param);
	printk(KERN_DEBUG "<%s> xdma_filter: *param 0x%x\n", MODULE_NAME, *(int *)param);

	if (XILINX_DMA_IP_MASK < chan->private)
	{
		printk(KERN_DEBUG "<%s> xdma_filter: *chan->private 0x%x\n", MODULE_NAME, *((int *)chan->private));
...
static void xdma_probe(void)
{
	...
	for (;;) {
		printk(KERN_DEBUG "<%s> xdma_probe: request tx channel %d\n", MODULE_NAME, num_devices);

		// In kernel code chan->common.private = (chan->direction & 0xFF) | (chan->feature & XILINX_DMA_IP_MASK);
		...
		printk(KERN_DEBUG "<%s> xdma_probe: request rx channel %d\n", MODULE_NAME, num_devices);
		...
		if (!tx_chan && !rx_chan) {
			...
			printk(KERN_DEBUG "<%s> xdma_probe: xdma_add_dev_info %d\n", MODULE_NAME, num_devices);
...
static int __init xdma_init(void)
{
	...
	printk(KERN_DEBUG "<%s> init: now allocate mmap area\n", MODULE_NAME);
	/* allocate mmap area */
	xdma_addr =
	...
	printk(KERN_DEBUG "<%s> init: now run probe\n", MODULE_NAME);
	/* hardware setup */
	xdma_probe();
